...
export const checkSeenEmailInDynamoDB = async (
  userName: string,
  s3Arn: string
) => {
...
export const updateHtmlImages = async (
  messageId: string,
  parsedEmail: ParsedMail
) => {
  const imageSources = parsedEmail.html
    ? extractImageSourcesFromHtml(parsedEmail.html)
    : [];
  ...
  for (const img of imageSources) {
    ...
    try {
      if (img.type === "cid") {
        ...
        const filename =
          url.pathname.split("/").pop()?.split("?").shift() || uuidv4();
      ...
      logger.error(
        `Error uploading image to S3: ${JSON.stringify({ messageId, src: img.src }, null, 2)}`
      );
      logger.error(e);
    ...
    if (s3Key) {
      proxyUrl =
        Resource["cdn_distribution"].distributionDomainName + "/" + s3Key;
...
export async function fetchFromImgproxyAndStore({
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch from imgproxy: ${res.status} ${res.statusText} ${imgproxyUrl} ${baseUrl}`
    );
  }
  ...
  contentType =
    res.headers.get("Content-Type") || (await imageType(buffer))?.mime;
  await s3.send(
    new PutObjectCommand({
      Bucket: bucketName,
      Key: s3Key,
